...
export default function ResultPage() {
    ...
    const { gameWon, userName, rewards, finalTime, tokens, usedClues, levelsCleared, solvedQuestionIds } = useGame();

    useEffect(() => {
        // If arrived here without playing (no win), redirect home
        if (!gameWon) {
        ...
        <div className="min-h-screen bg-black text-green-500 font-mono flex flex-col items-center justify-center p-4 relative overflow-hidden">
            ...
            <div className="max-w-5xl w-full z-10 border border-green-500/30 bg-black/80 backdrop-blur-md p-8 md:p-12 shadow-[0_0_100px_rgba(34,197,94,0.2)] animate-in slide-in-from-bottom duration-700">
                ...
                <header className="text-center mb-12 border-b border-green-900/50 pb-8">
                    <div className={`inline-block px-4 py-1 ${gameWon ? 'bg-green-900/20 text-green-400 border-green-500/30' : 'bg-yellow-900/20 text-yellow-400 border-yellow-500/30'} text-xs font-mono tracking-[0.3em] mb-4 border rounded-full`}>
                        {gameWon ? 'OPERATION SUCCESSFUL' : 'SESSION COMPLETED'}
                    </div>
                    <h1 className={`text-5xl md:text-8xl font-black tracking-tighter ${gameWon ? 'text-white drop-shadow-[0_0_15px_rgba(34,197,94,0.8)]' : 'text-zinc-300'} mb-2`}>
                        REPORT
                    </h1>
                    <p className="text-red-500 font-bold uppercase tracking-widest text-sm md:text-lg">
                        TEAM: <span className={gameWon ? 'text-green-400' : 'text-yellow-400'}>{userName || 'UNKNOWN'}</span>
                    </p>
                </header>

                <div className="flex justify-between">
                    {/* Primary Stats */}
                    <div className="space-y-8 flex flex-col md:flex-row justify-center gap-8 md:gap-16">
                        <div className="bg-zinc-900/30 border-t-4 border-green-500 p-6 relative overflow-hidden group hover:bg-zinc-900/50 transition-colors">
                    ...
                    <div className="bg-zinc-900/30 border-t-4 border-yellow-500 p-6 relative overflow-hidden group hover:bg-zinc-900/50 transition-colors">
                        ...
                        <div className="text-yellow-600/60 text-lg mt-2 font-mono">
                            POINTS ACCUMULATED
                        </div>
                    </div>

                    {/* LEVELS CLEARED */}
                    <div className="bg-zinc-900/30 border-t-4 border-blue-500 p-6 relative overflow-hidden group hover:bg-zinc-900/50 transition-colors">
                        <div className="absolute top-0 right-0 p-2 text-blue-800 opacity-20">
                        ...
                        <div className="text-zinc-500 text-sm font-bold uppercase tracking-widest mb-1">Levels Cleared</div>
                        <div className="text-5xl md:text-6xl font-black text-blue-500 font-mono tracking-tight">
                            {levelsCleared} <span className="text-2xl text-zinc-500">/ {levels.length}</span>
                        </div>
                        <div className="text-blue-600/60 text-lg mt-2 font-mono">
                            {solvedQuestionIds.filter(id => !usedClues[id]?.includes('skip')).length} QUESTIONS SOLVED
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-16 max-w-4xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {levels.flatMap((l, lIdx) => l.questions.map(q => ({ ...q, lIdx }))).map((q) => {
                            ...
                            } else if (!solvedQuestionIds.includes(q.id)) {
                                if (clues.length > 0) {
                                    if (clues.includes('hard')) {
                                        status = "UNSOLVED (HARD CLUE)";
                                        color = "text-orange-500";
                                        bg = "bg-orange-900/10 border-orange-900/30";
                                    } else {
                                        status = "UNSOLVED (EASY CLUE)";
                                    ...
                                    status = "NOT SOLVED";
                                    color = "text-zinc-600";
                                    bg = "bg-zinc-900/20 border-zinc-800";
                                }
                            } else if (clues.includes('hard') && clues.includes('easy')) {
                                status = "SOLVED (EASY + HARD)";
                                color = "text-orange-400";
                                bg = "bg-orange-900/20 border-orange-900/40";
                            } else if (clues.includes('hard')) {
                                status = "SOLVED (HARD CLUE)";
                                color = "text-orange-500";
                                bg = "bg-orange-900/10 border-orange-900/30";
                            ...
                            return (
                                <div key={q.id} className={`flex justify-between items-center p-3 border rounded ${bg}`}>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className={`text-sm truncate max-w-[200px] ${status === 'NOT SOLVED' ? 'text-zinc-600' : 'text-zinc-300'}`} title={q.q}>
                                            {q.q.substring(0, 30)}...
                                        </div>
                                    </div>
